...
function showNotification(message) {
    ...
    console.log(notification); // Should log the element, not null
    if (notification) {
...
// Use showNotification('Article saved!') instead of alert('Article saved!');
...
document.getElementById('save-article').addEventListener('click', function() {
    ...
    if (!isArticleSaved) {
        ...
        // And also this one in case the article is already saved.
        showNotification('This article is already saved!');
    }

    // Make sure we always go back to the welcome screen, regardless of notification.
...
document.getElementById('clear-topics').addEventListener('click', function() {
    ...
    const savedArticlesContainer = document.getElementById('saved-articles');
    savedArticlesContainer.innerHTML = '';
    showNotification('All saved topics have been cleared.');
    resetToWelcomeScreen();
});
