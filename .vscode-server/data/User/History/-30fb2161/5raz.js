...
async function generateQuiz() {
    let savedArticles = JSON.parse(localStorage.getItem('savedArticles')) || [];
    let quizQuestions = [];
    console.log('Generating quiz for', savedArticles.length, 'articles');

    for (let article of savedArticles) {
        try {
            let question = await fetchChatGPTResponse(`Generate a multiple-choice question based on the following summary: ${article.summary}`);
            quizQuestions.push({ question, title: article.title });
        } catch (error) {
...
function startQuiz() {
    ...
    document.getElementById('next-question').addEventListener('click', () => {
        // Logic to validate answer would be here - increment correctAnswers based on actual validation
        questionIndex++;
        if (questionIndex < quizQuestions.length) {
            showQuestion();
        } else {
...
function endQuiz() {
    let quizQuestions = JSON.parse(localStorage.getItem('quizQuestions')) || [];
    let correctAnswers = 0; // This would be calculated based on actual answers
