...
function generateFromBullet() {
    ...
    let firstPrompt = `Extract the main topic or entity from this detail: "${bulletText}"? For example, from the input 'Crisis management can save a company's reputation, like in the case of Johnson & Johnson's Tylenol contamination incident in 1982. The company's effective response saved its image.', the output should be: Johnson & Johnson's Tylenol contamination incident in 1982.  Your output should only include the topic as a standalone, so output Johnson & Johnson's Tylenol contamination incident in 1982., not like "Main topic:" or "The main topic is" `;
