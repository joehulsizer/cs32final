...
function generateFromBullet() {
    ...
    let firstPrompt = `Extract the main topic or entity from this detail: "${bulletText}"? that would be fitting for a Who or What question. For example, from the input 'Crisis management can save a company's reputation, like in the case of Johnson & Johnson's Tylenol contamination incident in 1982. The company's effective response saved its image.', the output could be: Johnson & Johnson's Tylenol contamination incident. or it could be: Johnson & Johnson.  Your output should only include the topic as a standalone, so output Johnson & Johnson's Tylenol contamination incident., not like "Main topic:" or "The main topic is" `;
