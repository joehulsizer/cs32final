...
function generateFromBullet() {
    ...
    // Generate a topic from the first selected bullet
    let bulletText = selectedBullets[0].value;
    let detailedTopicPrompt = `What is detailed information regarding: "${bulletText}"?`;
    
    fetchChatGPTResponse(detailedTopicPrompt).then(detailedSummary => {
        document.getElementById('article-summary').innerHTML = detailedSummary;  // Display the new detailed summary
    }).catch(error => {
        console.error('Error fetching detailed topic:', error);
        alert('Failed to generate detailed topic. Please try again.');
    });
}