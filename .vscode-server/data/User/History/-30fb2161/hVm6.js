...
document.getElementById('view-saved').addEventListener('click', function() {
    showLoadingScreen();  // Indicate loading process

    fetch('http://localhost:3000/api/topics')
        .then(response => response.json())
        .then(savedArticles => {
            const savedArticlesContainer = document.getElementById('saved-articles');
            savedArticlesContainer.innerHTML = '';  // Clear previous entries
            ...
            if (savedArticles.length === 0) {
                ...
                savedArticles.forEach(article => {
                    ...
                    titleElement.className = 'saved-article-title';

                    articleContainer.appendChild(titleElement);
                    savedArticlesContainer.appendChild(articleContainer);

                    // Add a button to view each full article
                    const viewButton = document.createElement('button');
                    viewButton.textContent = "View";
                    viewButton.className = "view-article-button";
                    viewButton.onclick = () => displayArticle(article);
                    articleContainer.appendChild(viewButton);
                });
            }

            hideLoadingScreen();  // Hide loading screen after displaying
            document.getElementById('welcome-screen').style.display = 'none';
            document.getElementById('saved-articles-screen').style.display = 'block';
        })
        .catch(error => {
            console.error('Error fetching topics:', error);
            alert('Error fetching saved articles. Please check the console for more details.');
            hideLoadingScreen();
        });
});
...
document.getElementById('save-article').addEventListener('click', function() {
    ...
    const article = { title, summary, image1, image2 };

    showLoadingScreen();  // Show the loading screen while processing
    ...
    .then(response => {
        hideLoadingScreen();  // Hide the loading screen after processing

        if (response.ok) {
            alert('Article saved!');
            resetToWelcomeScreen();  // Return to the main screen after successful save
        } else {
            console.error('Error saving article: Server responded with non-OK status.');
            alert('Error saving article. Please try again.');
        }
    })
    .catch(error => {
        console.error('Error saving article:', error);
        alert('Error saving article. Please check the console for more details.');
        hideLoadingScreen();
    });
});
