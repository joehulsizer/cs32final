...
document.getElementById('view-saved').addEventListener('click', function() {
    // Display the loading screen while fetching articles
    showLoadingScreen();
    ...
    fetch('http://localhost:3000/api/topics')
        ...
        .then(savedArticles => {
            ...
            savedArticlesContainer.innerHTML = '';  // Clear previous articles
            ...
            if (savedArticles.length === 0) {
                ...
                savedArticles.forEach((article, index) => {
                    ...
                    // Add view button to each article
            ...
            // Transition to the saved articles screen
            document.getElementById('welcome-screen').style.display = 'none';
            document.getElementById('saved-articles-screen').style.display = 'block';

            // Hide the loading screen after articles are fetched and displayed
            hideLoadingScreen();
        })
        .catch(error => {
...
document.getElementById('save-article').addEventListener('click', function() {
    ...
    // Display the loading screen while saving the article
    showLoadingScreen();
    ...
    .then(response => {
        hideLoadingScreen();  // Hide the loading screen regardless of the outcome

        if (response.ok) {
            alert('Article saved!');
        } else {
            response.text().then(text => alert(`Error saving article: ${text}`));
        }

        resetToWelcomeScreen();  // Return to the main screen after saving
    })
    .catch(error => {
